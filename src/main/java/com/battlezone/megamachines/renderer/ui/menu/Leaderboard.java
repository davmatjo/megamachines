...
public class Leaderboard implements Renderable {
    ...
    private void init() {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            var box = new Box(boxSize / 20 * 44, boxSize, x + 3 * width / 4 - boxSize / 2, boxTop + padding + (boxSize + padding) * (i), car.getColour(), t);
            boxes[i] = box;


            Label l = new Label(textForPosition(car.getPosition()), boxSize, 0, 0, Colour.WHITE);
            var labelWidth = l.getWidth();
            l.setPos(x + boxSize / 2 - labelWidth / 2, boxTop + padding + (boxSize + padding) * (i));
            labels[i] = l;
        }
